...
\begin{itemize}
\item Los elementos en nuestro programa se manejen con un código identificador interno oculto. 
\item Los usuarios utilicen otro u otros códigos para referirse a ellos al teclearlos, al mostrarlos en pantalla, en listados, etc.
\end{itemize}
...
Trabajando con los diez dígitos naturales 0,1,2,....,7,8,9 $\Rightarrow$ 10 caracteres, se dispone de $10^n$ códigos posibles. Por ejemplo, con códigos de 4 números de longitud, se pueden identificar $10^4 = 10.000 elementos$; con códigos de 5 números de longitud, se pueden identificar $10^5 = 100.000$  elementos; con códigos de 6 números de longitud, se pueden identificar $21^6 = 1.000.000$ de elementos; etc.
...
\vspace{1cm}
Trabajando con las consonantes mayúsculas B,C,D,F,....,X,Y,Z $\Rightarrow$ 21 caracteres, se dispone de $21^n$ códigos posibles. Por ejemplo, con códigos de 4 letras de longitud, se pueden identificar $21^4 = 194.481$ elementos; con códigos de 5 letras de longitud, se pueden identificar $21^5 = 4.084.101$ elementos; con códigos de 6 letras de longitud, se pueden identificar $21^6 = 85.766.121$  elementos; etc.
...
\item ``Modern Software Engineering: Doing What Works to Build Better Software Faster'' - David Farley -  Addison-Wesley (2022) $\rightarrow$ Los aspectos importantes en la forma de trabajar a la hora de desarrollar software. Trabajo iterativo, en pequeños pasos, con feedback rápido, con el paraguas de un buen juego de tests, con el soporte de una buena y clara arquitectura, refactorizando continuamente, integrando continuamente, automatizando los procesos de CI/CD,...

\includegraphics[width=4cm]{portada-ModernSoftwareEngineering-DavidFarley.jpg}
...
\item ``Unit Testing: Principles, Practices and Patterns'' - Vladimir Khorikov - Manning Publications (2020) $\Rightarrow$ Una explicación clara y detallada del uso de tests, junto con técnicas útiles y prácticas sobre cómo escribirlos.

\includegraphics[width=4cm]{portada-UnitTesting-VladimirKhorikov.jpg} 
...
\subsubsection{BBoM (``Big Ball of Mud'')}
El programa se compone de un solo bloque. Estructurado o no en funciones. Pero sin separaciones claras entre partes. Cualquier parte hace uso libre de cualquier otra.
...
\begin{center}
\fbox{
    \parbox{0.8\textwidth}{Cuidado con los falsos ``servicios''.\par
    Un servicio es una parte del sistema que realiza un determinado trabajo concreto de forma independiente, sin necesidad de otras partes para hacerlo.\par
    Si un ``servicio'' requiere la colaboración de unos cuantos otros ``servicios'' para hacer su trabajo, se ha de considerar que todos ellos son los que conforman realmente el servicio.}
}
\end{center}
\vspace{1cm}
...
Una aplicación puede escalar en dos sentidos:

\begin{itemize}

    \item Escalabilidad \textbf{vertical}: utilizando un ``trabajador'' con más capacidad, es decir añadiendo más recursos a la máquina. Por ejemplo, añadiendo más memoria, más CPU, más discos duros, más tarjetas de red, etc.

    \item Escalabilidad \textbf{horizontal}: utilizando más ``trabajadores'', es decir añadiendo más máquinas al sistema. Por ejemplo, añadiendo más servidores, más nodos, etc.

\end{itemize}

Es imprescindible tener bien claros los conceptos de:

\begin{description}

    \item[lógica monolítica:] todas las partes del código están fuertemente acopladas unas a otras, se hace difícil separarlas.

    \item[lógica distribuida:] el código está organizado en módulos bien definidos, con claros interfaces entre unos y otros.
    
    \item[implementación física monolítica:] todas las partes corren en una sola máquina.
    
    \item[implementación física distribuida:] diferentes partes corren en diferentes máquinas, esas máquinas están enlazadas entre sí a través de conexiones de red.

\end{description}

Las lógicas distribuidas pueden desplegarse sobre una implementación física monolítica o sobre una distribuida. Y pueden escalarse tanto vertical como horizontalmente. Esta última, si es que la distribución de la lógica permite beneficiarse de tener más instancias de algunos de los módulos de la aplicación; claro está.

Sin embargo, las lógicas monolíticas solo pueden ser desplegadas sobre implementaciones físicas monolíticas. Es decir, solo pueden escarlar verticalmente. Cuando se intentan escalar horizontamente, la sobrecarga de las comunicaciones a través de la red no hace sino empeorar su rendimiento.
