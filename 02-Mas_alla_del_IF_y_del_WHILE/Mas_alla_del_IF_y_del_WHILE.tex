...
Hasta hace unos años, esta multitarea era virtual (una sola unidad de procesamiento atendía todas las tareas) y suponía una carga adicional al sistema (la sobrecarga de congelar una tarea para reactivar otra, y de hacerlo a una velocidad tal que todas las tareas parecieran estar funcionando a la vez). De ahí que solo se utilizara cuando era estrictamente necesario.

Pero hoy en día, la multitarea es real. El hardware de computación está llegando al límite de frecuencia de reloj y de optimización interna de cada unidad. Para seguir mejorando, los fabricantes han optado por integrar múltiples unidades de computación (cores) en cada procesador y multiples (co)procesadores en cada sistema.

Es decir, la programación de software concurrente o paralelo está cobrando cada vez más importancia. Siendo hoy en día imprescindible para aprovechar toda la capacidad de los sistemas hardware.

\section{Definiciones}
\begin{description}

\item[Batch:] Se prepara una lista de trabajos. Para irlos ejecutando uno a continuación de otro, según la secuencia preprogramada.

\item[Secuencial:] Se va realizando un trabajo tras otro, según van llegando. Un trabajo no comienza hasta que no termina el anterior.

\item[Concurrente:] Múltiples trabajos se van solapando en el tiempo, pero su ejecución no es necesariamente simultánea.
\\Se suele utilizar cuando se necesita garantizar un tiempo máximo de respuesta para cada trabajo. Es decir, cuando un trabajo no puede obligar a todos los demás a permanecer inativos hasta que termine.
\\Se puede realizar con varias unidades de procesamiento (repartiendo trabajos entre ellas) o también con una sola unidad de procesamiento (alternando entornos de ejecución).

\item[Paralelo:] Multiples trabajos ejecutandose de forma simultánea. Pudiendo ser algunos de ellos partes de una tarea mayor.
\\Se suele utilizar para completar los trabajos en menos tiempo.
\\Implica disponer de varias unidades de procesamiento.

\item[Distribuido:] Los trabajos se realizan en varias máquinas diferentes, cada una de ellas independiente de las demás. Conectadas entre sí a través de una red de comunicaciones, para intercambiar información y coordinar trabajos.
...
\section{Estrategias para repartir y colaborar}
...
Mirandolo desde el punto de vista de las dependencias. Los distintos trabajos pueden necesitar:

\begin{itemize}

  \item Usar a turnos un recurso compartido que solo puede ser usado por un trabajo a la vez.
  \item Actualizar un estado común utilizado por varios trabajos.
  \item Compartir información con otros trabajos.

\end{itemize}

Para lidiar con recursos o estados compartidos, se suelen utilizar o bien bloqueos que reservan el uso para un solo trabajo cada vez durante todo el tiempo que necesite para completarse, o bien operaciones atómicas que puedan ser realizadas sin dar tiempo a otro trabajo a interponerse.

Para lidiar con la compartición de información, se suele utilizar o bien esperas (await) por parte de un trabajo al otro al que ha pedido la información, o bien mensajes que un trabajo envia (send-and-forget) cuando termina su parte y que otro recibe/procesa cuando pueda leerlo.

Mirándolo desde el punto de vista de la coordinación. Los distintos trabajos pueden interactuar entre sí:
...
\section{Salvaguardas para evitar problemas al usar recursos compartidos}

Como se ha comentado, es mejor evitar recursos compartidos que se hayan de utilizar en exclusividad. Pero en la práctica no siempre es posible evitarlos. 

Cuando se utilizen, es básico garantizar que unas tareas no interfieran ni estropeen el trabajo de otras. Los problemas más habituales suelen ser:

\begin{itemize}
  \item Una tarea acapara algo y no lo libera. Deteniendo indefinidamente el trabajo de otras que están a la espera de ese algo.
  \item Una tarea espera algo de otra. Pero no se entera de que esa otra tiene problemas y no se lo va a poder suministrar; quedando indefinidamente a la espera.
  \item Una tarea lee unos datos y trabaja en base a ellos. Sin percatarse que otra tarea, entremedio, ha alterado dichos datos; invalidando el trabajo que está realizando.
\end{itemize}

\subsection{detección y recuperación}
La forma más básica de soslayar problemas es tener algún mecanismo de vigilancia. Que:
\begin{itemize}
  \item O bien detecta el problema y lo notifica a las partes afectadas. Para que cada una actue en consecuencia.
  \item O bien establece unos tiempos máximos de espera (timeout). De tal forma que, al alcanzarse el tiempo máximo establecido, la tarea se da por perdida y se recupera el uso o se actua en consecuencia para permitir que el sistema continue trabajando.
\end{itemize}

Habitualmente se suele complementar con algún mecanismo de reintento. Siempre y cuando sea viable reintentar las tareas fallidas.

Con los reintentos, dos problemas muy típicos son:
\begin{itemize}
  \item Malinterpretar un reintento como una nueva operación, duplicando resultados. Para evitarlo, se suele recurrir a identificar de algún modo unívoco cada operación, de tal forma que tanto la operación inicial como sus reintentos tengan la misma identificación y pueda saberse si está o no ya realizada.
  \item Reintentos masivos en un sistema congestionado, provocando una congestión aún mayor. Para evitarlo, se suele recurrir a lanzar cada reintento con un cierto retraso aleatório; siendo ese retraso progresivamente mayor para un segundo, tercer, cuarto,... reintento de una misma operación.
\end{itemize}
...
\item Mezcolanza (Big Ball of Mud):  no hay límites claros entre distintas responsabilidades y distintos dominios; las partes hacen un amplio uso directo unas de otras, según convenga; el sistema evoluciona ``como se pueda''\ldots
...
\begin{footnotesize}
Una posible muestra de algunos de los conceptos citados puede verse en
\\ \url{https://github.com/JuanMuruaOlalde/Hotel}
\end{footnotesize}
\begin{scriptsize}
...
\begin{footnotesize}
Otras posibles muestras más pequeñas podrian ser \\ \url{https://github.com/JuanMuruaOlalde/DesarrolloDeSoftware/tree/main/02-Mas_alla_del_IF_y_del_WHILE/soluciones_java/MVC_springboot} y \\ \url{https://github.com/JuanMuruaOlalde/MeteoAPI}
\end{footnotesize}
