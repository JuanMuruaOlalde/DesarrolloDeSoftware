...
\end{itemize}
La descripción de los argumentos de una función es lo que se suele denominar la `signatura' de esa función.
...
Aprovecho para recordar la importancia de poner nombres claros y descriptivos a todas las variables, argumentos y funciones. De esta forma, se facilita la legibilidad del código. (En un código bien escrito se deberia comprender lo que hace con solo leerlo, sin necesidad de ``comentarios explicativos'' adiccionales).


\subsection{Organizar bien el código}

Normalmente suele haber grupos de funciones con una cierta relación entre sí. Es importante ir distinguiendo de forma clara estos grupos a medida que vamos construyendo el programa. Para ir poniendo las funciones relacionadas entre sí dentro de un mismo ``trozo'' del programa.
...
Las funciones dentro un mismo ``trozo'' se suelen poder llamar directamente entre sí. Pero para llamar a una función residente en un ``trozo'' desde otro ``trozo'', suele requerir hacer algo ``un poco especial'' (como, por ejemplo, `importar' previamente el módulo donde reside).

De esta forma, van quedando claras las dependencias entre partes. 

Siendo el objetivo el limitar esas dependencias a las mínimas imprescindibles. Teniendo solo aquellas dependencias que sean lógicas según el trabajo a realizar por cada ``trozo'' del programa.

Del mismo modo que cada función del programa ha de ser consistente y tener una única ``responsabilidad'' clara y definida.  Cada ``trozo'' (módulo, clase,\ldots) también ha de ser coherente y tener una única misión clara y definida.

Lo ideal es que cada ``trozo'' tenga unos puntos de acceso bien definios y que cada uno de esos puntos de acceso proporcione unos servicios o respuestas concretos. Es lo que se suele denominar `\textbf{interfaces}' entre parte y parte.

Si estas interfaces están bien definidas y se mantienen estables en el tiempo, permiten modificar/ampliar (o sustituir) unas partes del programa sin afectar a las otras.

Resultando en un programa sencillo de mantener y de evolucionar.

\vspace{0.5cm}
nota: 

Este es el aspecto más difícil de dominar en la programación. 

Llegar a pensar en todo el programa como un conjunto coherente, formado por múltiples partes. Cada parte con una responsabilidad concreta, clara y definida. Pero todas ellas trabajando conjuntamente para conseguir los fines (globales) que se persiguen.

No pocas veces, la presión del momento nos lleva a hacer ``algo que funcione''. Sin pararnos a pensar en cómo encaja ese algo con la globalidad (la arquitectura) del sistema.
\\Si luego no volvemos sobre nuestros pasos a refactorizar ese ``algo que funciona'', para integrarlo bien dentro del conjunto. Acabaremos con el temido `Big Ball of Mud'.


\subsection{Refactorizar de forma continua (y sin miedo)}

Refactorizar: modificar el código sin afectar a lo que hace (es decir, rescribirlo para que quede mejor, pero manteniendo su funcionalidad tal cual)
